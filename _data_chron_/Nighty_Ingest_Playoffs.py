...
class NightlyIngestPlayoffs:
    ...
    def __init__(self):
        ...
        self.cumulative_col_widths = [7, 6, 4, 4, 24, 32, 2, 5, 6, 5, 5, 4, 7, 4, 5, 5, 4, 6, 6, 4, 4, 6, 4, 6, 5, 3]
        self.cumulative_col_names = ['IDENT', 'SCOPE', 'team', 'roster_sts', 'name', 'team_name_2', 'gms', 'gms_start', 'minutes',
                                     'fg', 'fga', 'fg3', 'fg3a', 'ft', 'fta', 'off_rbd', 'def_rbd', 'tot_rbd', 'ast', 'pf', 'dq', 'stl', 'tov', 'blk', 'pts', 'hi']
    ...
    def cumulative(self, page):
        ...
        for row in data_dict:
            ...
            keys = ', '.join(row.keys())
            values = tuple(row.values())
            query = f"INSERT IGNORE INTO playoffs_cumulative_player ({keys}) VALUES {values}"
            # Insertion in DB
            try:
                cursor.execute(query)
            except (TimeoutError, ConnectionError, TypeError) as err:
